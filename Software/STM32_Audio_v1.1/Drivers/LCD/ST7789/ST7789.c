...
uint16_t DisplayBuff[ST7789_WIDTH * HOR_LEN];
#endif
...
/*
 * This function use for TouchGFX generated 16 bits color is little endian
 * need to swap byte high & low of array bitmap
 */
void ST7789_SwapByteHighLow(uint8_t *data, uint32_t size)
{
  uint8_t temp;
  while (size > 0)
  {
...
void ST7789_ChangeStringToLeft(char *dest ,const char *src, FontDef font)
{
  uint8_t i, cursor_split;
  uint8_t len = strlen(src);
  char row[2][50] = {0};
  uint8_t number_char_per_row = ST7789_WIDTH / font.width;
//  memset(dest, 100, 0);
  for(i = 0; i < len; i++)
  {
    // copy and convert '-' to ' '
...
//void ST7789_WriteStringToMiddle(uint16_t x, uint16_t y, const char *str, uint16_t len, FontDef font, uint16_t color, uint16_t bgcolor)
//{
//  uint16_t x_start;
//  uint16_t y_start;
//  uint16_t x_size = font.width * len;
//  uint16_t y_size = font.height;
...
void ST7789_Test(void)
{
	...
	//HAL_Delay(3000);
	ST7789_DrawFilledCircle(120, 120, 90, GRAY);
	ST7789_DrawFilledCircle(120, 120, 80, WHITE);

	//ST7789_DrawTriangle(30, 30, 30, 70, 60, 40, RED);
//  ST7789_WriteString(10, 10, "Initialize LCD Done1", Font_11x18, BRED, WHITE);
//  ST7789_WriteString(10, 40, "Now playing Music...", Font_11x18, BLUE, WHITE);
}
